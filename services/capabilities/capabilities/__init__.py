"""The capabilities package is a :py:mod:`nameko` microservice which describes available capabilities of the backend.

To run this microservice it needs to connect to a message broker - `RabbitMQ`_ is used here. Besides this server no
other components are required for a minimal setup - the service neither connects to an other microservices nor uses a
database.
...
that some settings are used BEFORE validation can take place - e.g. connection to `RabbitMQ`_ - in this case a missing
