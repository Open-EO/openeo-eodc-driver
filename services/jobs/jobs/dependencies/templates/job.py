...
class Job(BaseTemplate):
    ...
    def __init__(self, template_id, imagestream, cpu_request, cpu_limit, mem_request, mem_limit):
    ...
    def extract_selfLinks(self, response, api_connector):
        ...
        pods = api_connector.request("openshift", "get", "/api/v1/namespaces/{0}/pods")
    ...
    def get_metrics(self, api_connector):
        ...
        if "containerStatuses" in pod["status"]:
            ...
            metrics = api_connector.request("hawkular", "get", "/metrics?tags=pod_name:" + pod_name)

            for metric in metrics:
                m_name = metric["tags"]["descriptor_name"]
                if m_name in self.__METRICS:
                    m_path = "/{0}s/{1}/data".format(metric["type"], quote(metric["id"], safe=''))
