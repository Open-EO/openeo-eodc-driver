...
from tests.utils import get_configured_job_service, get_dag_path, get_random_user_id, load_json
...
class TestCreateJob:
    ...
    def test_basic(self, db_session: Session) -> None:
        ...
        assert db_session.query(Job).filter(Job.user_id == user_id).filter(Job.id == results_job_id).count() == 1
        ...
        job_service.processes_service.put_user_defined.assert_called_once_with(
            user_id=user_id, process_graph_id="pg_id", **job_data["process"]
        )
        job_service.files_service.setup_jobs_result_folder.assert_called_once_with(
            user_id=user_id, job_id=results_job_id)
